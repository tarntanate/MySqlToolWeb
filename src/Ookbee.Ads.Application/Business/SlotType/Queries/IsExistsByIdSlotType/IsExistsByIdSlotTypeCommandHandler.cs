...
{
    public class IsExistsByIdSlotTypeCommandHandler : IRequestHandler<IsExistsByIdSlotTypeCommand, HttpResult<bool>>
    {
        private OokbeeAdsMongoDBRepository<SlotTypeDocument> SlotTypeMongoDB { get; }

        public IsExistsByIdSlotTypeCommandHandler(OokbeeAdsMongoDBRepository<SlotTypeDocument> slotTypeMongoDB)
        {
            SlotTypeMongoDB = slotTypeMongoDB;
        }

        public async Task<HttpResult<bool>> Handle(IsExistsByIdSlotTypeCommand request, CancellationToken cancellationToken)
        {
