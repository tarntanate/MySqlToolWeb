...
{
    public class IsExistsByIdPricingModelCommandHandler : IRequestHandler<IsExistsByIdPricingModelCommand, HttpResult<bool>>
    {
        private OokbeeAdsMongoDBRepository<PricingModelDocument> PricingModelMongoDB { get; }

        public IsExistsByIdPricingModelCommandHandler(OokbeeAdsMongoDBRepository<PricingModelDocument> pricingModelMongoDB)
        {
            PricingModelMongoDB = pricingModelMongoDB;
        }

        public async Task<HttpResult<bool>> Handle(IsExistsByIdPricingModelCommand request, CancellationToken cancellationToken)
        {
