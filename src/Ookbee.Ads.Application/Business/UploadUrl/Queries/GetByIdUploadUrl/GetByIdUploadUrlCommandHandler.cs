...
namespace Ookbee.Ads.Application.Business.UploadUrl.Queries.GetByIdUploadUrl
{
    public class GetByIdUploadUrlCommandHandler : IRequestHandler<GetByIdUploadUrlCommand, HttpResult<UploadUrlDto>>
    {
        ...
        public GetByIdUploadUrlCommandHandler(OokbeeAdsMongoDBRepository<UploadUrlDocument> uploadUrlMongoDB)
        {
            UploadUrlMongoDB = uploadUrlMongoDB;
        }

        public async Task<HttpResult<UploadUrlDto>> Handle(GetByIdUploadUrlCommand request, CancellationToken cancellationToken)
        {
