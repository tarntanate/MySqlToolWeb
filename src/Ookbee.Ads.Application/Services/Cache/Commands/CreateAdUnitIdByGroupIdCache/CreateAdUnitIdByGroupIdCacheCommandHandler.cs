...
namespace Ookbee.Ads.Application.Services.Cache.Commands.CreateAdUnitIdByGroupIdCache
{
        ...
        public async Task<Unit> Handle(CreateAdUnitIdByGroupIdCacheCommand request, CancellationToken cancellationToken)
        {
            ...
            do
            {
                ...
                if (items.HasValue())
                {
                    ...
                    {
                        var redisKey = CacheKey.UnitIdListByGroupId(adGroupId);
                        var redisValues = items.Where(item => item.AdGroupId == adGroupId).Select(item => (RedisValue)item.Id).ToArray();
