...
namespace Ookbee.Ads.Application.Services.Cache.AdUnitCache.Commands.CreateAdUnitIdByPlatformCache
{
    public class CreateAdUnitIdByPlatformCacheCommandHandler : IRequestHandler<CreateAdUnitIdByPlatformCacheCommand>
    {
        ...
        public async Task<Unit> Handle(CreateAdUnitIdByPlatformCacheCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                next = false;
                var predicate = AdUnitFilter.Available();
                        ...
                        foreach (var platform in platforms)
                        {
                            var obj = items.Where(item => item.AdGroupId == adGroupId).Select(item => new AdUnitCacheDto()
                            {
                                ...
                                Analytics = item.AdNetwork.ToUpper() == "OOKBEE"
                                    ? null
                                    : new AdUnitAnalyticsCacheDto()
                                {
                                    Clicks = new List<string>() {
                                        $"{baseUrl}/api/units/{item.Id}/stats?type={AdStatsType.Click}&platform={platform}&publisherId={item.PublisherId}".ToLower()
                                    },
                                    Impressions = new List<string>() {
                                        $"{baseUrl}/api/units/{item.Id}/stats?type={AdStatsType.Impression}&platform={platform}&publisherId={item.PublisherId}".ToLower()
                                    }
                                },
                                Name = item.AdNetwork
                            });
                            ...
                            cacheObj.Data.Items = obj.Where(x => !string.IsNullOrEmpty(x.Id)).ToList();
